...
4. **Termination:** The task is fully processed when:
   - Implementation is complete, committed, and the issue is marked Done, OR
...
3. If no specific issue:
   ...
   - Merge both result sets into a single list.
...
**If no eligible issue is found in In Progress or In Review:**
Stop and report:
> "No task issues (title starting with 'Task:') found In Progress or In Review for project <name>. Nothing to implement."
